...
\caption{(Color online) Three SCMs which are normalized by $\mathcal{C}_{max}$ and $\mathcal{C}_{min}$  at a given $\mathcal{H}$, for the logistic map.  \label{fig:CHlogCmax}}
\end{figure*}
}
...
{\color{red} For fBM, we can include another figure showing normalized complexity measures by the corresponding $\mathcal{C}_{max}$ and $\mathcal{C}_{min}$ as Fig. \ref{fig:CHlogCmax}. As the embedding dimension $D$ is increased, the dots are more aligned to $\mathcal{C}_{max}$ and extend further to the left half of the complexity--entropy plane. 
\begin{figure*}
        \centering
