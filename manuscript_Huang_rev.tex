...
The situation becomes even more challenging when our modified SCMs are considered, since they are based on transition frequencies instead of pattern frequencies. For $D!$ different ordinal patterns for embedding dimension $D$, there may exist $D!^2$ possible source--destination pairs. However, if we consider $\tau=1$ and transitions corresponding to shifting the time index by $\Delta t=1$ time step only, from the $D$ components of the source permutation $D-1$ also contribute to the corresponding destination pattern without changing their mutual order relationships \cite{BorgesAMC2019,PessaPRE2019}. Hence, in this special case the new state contributing to the destination pattern can only lead to at most $D$ different ordinal patterns, i.e., we have at most $D\cdot D!$ different pattern transitions. However, in other situations with $\tau>1$ and/or larger time shifts $\Delta t$ between successive ordinal patterns to be considered, up to $D!^2$ different pairs of patterns and, hence, transitions could occur. In general, we conclude that the far higher number of pattern transitions as compared to patterns themselves puts severe restrictions to the choice of $D$ in practical applications of the new SCMs in the context of time series of a given length $L$.
}
...
\caption{\small{(Color online) Same as in Fig.~\ref{fig:CElogistic}, but for fractional Brownian motion with a Hurst exponent $H \in (0, 1)$ varying with a step size of $\Delta H=0.05$. Note that the normalized entropy is generally decreasing with increasing $H$ (red arrow). (a, d) $\mathcal{C}_O$ versus $\mathcal{H}_O$, (b, e) $\mathcal{C}_{W}$ versus $\mathcal{H}_{W}$, and (c, f) $\mathcal{C}_{E}$ versus $\mathcal{H}_{E}$. (a-c) for $D = 4, 5$, and (d-f) for $D = 6$. }  \label{fig:CEfBm}}
\end{figure*}

\begin{figure*}
	\centering 
	\includegraphics[width=2\columnwidth]{CompEntropy_fGn.pdf}
\caption{\small{(Color online) Same as in Fig.~\ref{fig:CElogistic}, but for fractional Gaussian noise with a Hurst exponent $H \in (0, 1)$ varying with a step size of $\Delta H=0.05$. Note that only a small region of the complexity--entropy planes is covered in this case. (a, d) $\mathcal{C}_O$ versus $\mathcal{H}_O$, (b, e) $\mathcal{C}_{W}$ versus $\mathcal{H}_{W}$, and (c, f) $\mathcal{C}_{E}$ versus $\mathcal{H}_{E}$. (a-c) for $D = 4, 5$, and (d-f) for $D = 6$. }  \label{fig:CEfGn}}
\end{figure*}
...
Especially for the new OPTN based SCMs, we find a partial overlap between the entropy values of fBm and the logistic map, while the associated SCM values are however clearly distinct except for the \textcolor{red}{fBm realizations} with the lowest entropy \textcolor{red}{values}, which correspond to the most nonstationary situations ($H$ close to 1), \textcolor{red}{which can be further seen in the Supplementary Material Fig.~S7. In general, we observe for all three complexity--entropy planes of fBm that with increasing $D$, the associated range of entropy values extends further towards the left (lower entropies). Taken both, upward and leftward shift of the complexity--entropy curves individually may therefore enhance the risk of confusing deterministic-chaotic and stochastic processes in the complexity--entropy plane even with our alternative SCMs. However, our results indicate that} combining information on the position in the complexity--entropy plane and on the stationarity could allow a clear distinction between chaotic and stochastic dynamics even in extreme situations \textcolor{red}{(e.g., a large value of $H$)}. Notably, for the stationary fGn, all \textcolor{red}{three complexity--entropy planes} are confined to very high entropy and low complexity values \textcolor{red}{(see also Supplementary Material Fig.~S8)} -- a range where no combinations have been found for the new transition frequency based SCMs for the logistic map. 
...
\caption{\color{red}(Color online) Same as in Fig.~\ref{fig:CHlogCmax}, but for different realizations of fractional Brownian motion with varying Hurst exponent $H$. (a) $\mathcal{C}_O^N$, (b) $\mathcal{C}_W^N$, and (c) $\mathcal{C}_E^N$. \label{fig:CHfbmCmax}}
\end{figure*}
...
\caption{\small{Sample segments of the temperature recordings from the flow experiment: (a) SW, (b) AV-2, (c) AV-3 and (d) MAV. The vertical axis \textcolor{red}{describes the} temperature $T$. } \label{tseriesFluid}}
\end{figure}
...
\caption{\color{red}\small{Box plots for OPTN based entropy and SCM characteristics for human ECG recordings, showing well separated medians for each subject group SR, VF and VT. Entropy and SCM scores are shown for embedding dimensions of $D = 4, 5, 6$. (a) $\mathcal{H}_O$ and (b) $\mathcal{C}_O$; (c) $\mathcal{H}_W$ and (d) $\mathcal{C}_W$; (e) $\mathcal{H}_E$ and (f) $\mathcal{C}_E$. } \label{fig:ecg}}
\end{figure*}
...
See Supplementary material: Sec.~SM1 with Figs.~S1 to S3 for the complexity--entropy planes when the self-loops are included in the transition matrix; Sec.~SM2 with Figs.~S4 and S5 for transition curves for the logistic map and the chaotic R\"ossler system when embedding parameters are $D=6$ and $\tau=1$; Sec.~ SM3 with Fig.~S6 for the linear increase of the maximum complexity $\mathcal{C}_{max}$ versus entropy curve for sufficiently large embedding dimension. However, this linear correlation in general is not expected by fractional Brownian motion and fractional Gaussian noise Sec.~ SM3 with Figs.~S7 and S8. 

\section*{Acknowledgements}
Parts of this work have been financially supported by the National Natural Science Foundation of China (grant no. 11872182, 11972288, 11835003, 11875132, 61973086), the Natural Science Foundation of Shanghai (grant no. 18ZR1411800). JZ and YZ further acknowledge support by the Shanghai Municipal Science and Technology Major Project and ZJ Lab (grant no. 2018SHZDZX01). The authors thank J. B. Borges and O. A. Rosso for sharing algorithms for computing $\mathcal{C}_{min}$ and $\mathcal{C}_{max}$\textcolor{red}{, and two anonymous reviewers for helpful comments on the original version of this manuscript}. 
...
\bibitem [{\citenamefont {Bates}\ and\ \citenamefont
  {Shepard}(1993)}]{batesPLA1993}%
  \BibitemOpen
  \bibfield  {author} {\bibinfo {author} {\bibfnamefont {J.~E.}\ \bibnamefont
  {Bates}}\ and\ \bibinfo {author} {\bibfnamefont {H.~K.}\ \bibnamefont
  {Shepard}},\ }\bibfield  {title} {\enquote {\bibinfo {title} {Measuring
  complexity using information fluctuation},}\ }\href {\doibase
  https://doi.org/10.1016/0375-9601(93)90232-O} {\bibfield  {journal} {\bibinfo
   {journal} {Phys. Lett. A}\ }\textbf {\bibinfo {volume} {172}},\ \bibinfo
  {pages} {416 -- 425} (\bibinfo {year} {1993})}\BibitemShut {NoStop}%
...
\bibitem [{\citenamefont {Pessa}\ and\ \citenamefont
  {Ribeiro}(2019)}]{PessaPRE2019}%
  \BibitemOpen
  \bibfield  {author} {\bibinfo {author} {\bibfnamefont {A.~A.~B.}\
  \bibnamefont {Pessa}}\ and\ \bibinfo {author} {\bibfnamefont {H.~V.}\
  \bibnamefont {Ribeiro}},\ }\bibfield  {title} {\enquote {\bibinfo {title}
  {Characterizing stochastic time series with ordinal networks},}\ }\href
  {\doibase 10.1103/PhysRevE.100.042304} {\bibfield  {journal} {\bibinfo
  {journal} {Phys. Rev. E}\ }\textbf {\bibinfo {volume} {100}},\ \bibinfo
  {pages} {042304} (\bibinfo {year} {2019})}\BibitemShut {NoStop}%
\bibitem [{\citenamefont {Kowalski}\ \emph {et~al.}(2007)\citenamefont
  {Kowalski}, \citenamefont {Martín}, \citenamefont {Plastino},\ and\
  \citenamefont {Rosso}}]{kowalskiPhyD2007}%
  \BibitemOpen
  \bibfield  {author} {\bibinfo {author} {\bibfnamefont {A.}~\bibnamefont
  {Kowalski}}, \bibinfo {author} {\bibfnamefont {M.}~\bibnamefont {Martín}},
...
\bibitem [{\citenamefont {L.-Ruiz}, \citenamefont {Mancini},\ and\
  \citenamefont {Calbet}(2010)}]{SMCbook2010}%
  \BibitemOpen
  \bibfield  {author} {\bibinfo {author} {\bibfnamefont {R.}~\bibnamefont
  {L.-Ruiz}}, \bibinfo {author} {\bibfnamefont {H.}~\bibnamefont {Mancini}}, \
  and\ \bibinfo {author} {\bibfnamefont {X.}~\bibnamefont {Calbet}},\
  }\bibfield  {title} {\enquote {\bibinfo {title} {A statistical measure of
  complexity},}\ }in\ \href {\doibase 10.2174/9781608057603113010012} {\emph
  {\bibinfo {booktitle} {Concepts and Recent Advances in Generalized
  Information Measures and Statistics}}},\ \bibinfo {editor} {edited by\
  \bibinfo {editor} {\bibfnamefont {A.~M.}\ \bibnamefont {Kowalski}}, \bibinfo
  {editor} {\bibfnamefont {R.~D.}\ \bibnamefont {Rossignoli}}, \ and\ \bibinfo
  {editor} {\bibfnamefont {E.~M.~F.}\ \bibnamefont {Curado}}}\ (\bibinfo
  {publisher} {Bentham Science Publishers},\ \bibinfo {year} {2010})\ pp.\
  \bibinfo {pages} {147--168}\BibitemShut {NoStop}%
...
\bibitem [{\citenamefont {Thiel}\ \emph {et~al.}(2004)\citenamefont {Thiel},
  \citenamefont {Romano}, \citenamefont {Read},\ and\ \citenamefont
  {Kurths}}]{thiel2004a}%
  \BibitemOpen
  \bibfield  {author} {\bibinfo {author} {\bibfnamefont {M.}~\bibnamefont
  {Thiel}}, \bibinfo {author} {\bibfnamefont {M.~C.}\ \bibnamefont {Romano}},
  \bibinfo {author} {\bibfnamefont {P.~L.}\ \bibnamefont {Read}}, \ and\
  \bibinfo {author} {\bibfnamefont {J.}~\bibnamefont {Kurths}},\ }\bibfield
  {title} {\enquote {\bibinfo {title} {{Estimation of dynamical invariants
  without embedding by recurrence plots}},}\ }\href {\doibase
  10.1063/1.1667633} {\bibfield  {journal} {\bibinfo  {journal} {Chaos}\
  }\textbf {\bibinfo {volume} {14}},\ \bibinfo {pages} {234--243} (\bibinfo
